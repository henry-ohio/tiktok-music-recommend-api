...
def get_common_id_column() -> Column:
    return Column(
        "id", psql.UUID(as_uuid=False),
...
def get_user_fk_column(
    col_name: str = "user_id",
    ondelete: str = None,
    *args,
    **kwargs
) -> Column:
    return get_fk_column(
        col_name,
        'users.id',
...
def get_common_columns(is_fk_user=False) -> List[Column]:
    return [
        ...
        Column("is_deleted", BOOLEAN, server_default=expression.false())
    ]


def get_common_language_column() -> Column:
